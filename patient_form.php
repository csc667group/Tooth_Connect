...
//    $password2 = mysql_real_escape_string($connection, trim($_POST['password2']));
    ...
    if (!empty($password1) && !empty($password2)&& ($password1 == $password2)  && !empty($email)
      ...
      $query2 = "SELECT * FROM dentist_data WHERE email = '$email'";
      $data = mysql_query($query);
      $data2 = mysql_query($query2);
      if (mysql_num_rows($data) == 0 && mysql_num_rows($data2) == 0) {
        // The email is unique, so insert the data into the database
        $query = "INSERT INTO patient_data (password,firstname,lastname,address,city,state,zipcode,email,phone)
            VALUES ( SHA('$password1'), '$firstname','$lastname','$address','$city','$state','$zipcode','$email','$phone')";
        mysql_query($query);
